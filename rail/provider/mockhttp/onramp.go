...
// OnRampProvider delegates on-ramp operations to the external mock provider HTTP service.
...
// NewOnRampProvider creates an OnRampProvider that delegates to the mock HTTP service.
func NewOnRampProvider(id string, pairs []domain.CurrencyPair, client *Client) *OnRampProvider {
	return &OnRampProvider{id: id, pairs: pairs, client: client}
}
...
func (p *OnRampProvider) GetQuote(ctx context.Context, req domain.QuoteRequest) (*domain.ProviderQuote, error) {
	if !p.supportsPair(req.SourceCurrency, req.DestCurrency) {
		return nil, fmt.Errorf("settla-rail: mockhttp on-ramp %s: unsupported pair %s→%s", p.id, req.SourceCurrency, req.DestCurrency)
	}
	...
	if err := p.client.doPost(ctx, "/api/onramp/quote", body, &resp); err != nil {
		return nil, err
	}
...
func (p *OnRampProvider) Execute(ctx context.Context, req domain.OnRampRequest) (*domain.ProviderTx, error) {
	if !p.supportsPair(req.FromCurrency, req.ToCurrency) {
		return nil, fmt.Errorf("settla-rail: mockhttp on-ramp %s: unsupported pair %s→%s", p.id, req.FromCurrency, req.ToCurrency)
	}
	...
	if err := p.client.doPost(ctx, "/api/onramp/execute", body, &resp); err != nil {
		return nil, err
	}
