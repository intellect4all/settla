...
const sizeClasses = computed(() => {
  const map: Record<string, string> = { xs: 'text-xs', sm: 'text-sm', base: 'text-base', lg: 'text-lg', xl: 'text-xl', '2xl': 'text-2xl' }
  return map[props.size]
})

const colorClass = computed(() => {
  const map: Record<string, string> = { default: 'text-surface-100', positive: 'text-emerald-400', negative: 'text-red-400', muted: 'text-surface-500' }
