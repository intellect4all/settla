<template>
  <div class="card overflow-hidden">
    <div v-if="title || searchable" class="flex items-center justify-between px-4 py-3 border-b border-surface-800">
      ...
      <input v-if="searchable" v-model="search" type="text" :placeholder="searchPlaceholder" class="input text-sm w-64" />
    </div>
      ...
      <table class="w-full text-sm">
          ...
          <tr class="border-b border-surface-800">
            ...
            >
              <span class="inline-flex items-center gap-1">
                {{ col.label }}
                <span v-if="col.sortable && sortKey === col.key" class="text-violet-400">{{ sortOrder === 'asc' ? '&#9650;' : '&#9660;' }}</span>
              </span>
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-if="paginatedRows.length === 0">
            <td :colspan="columns.length" class="px-4 py-8 text-center text-surface-500">{{ loading ? 'Loading...' : emptyMessage }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div v-if="totalPages > 1" class="flex items-center justify-between px-4 py-3 border-t border-surface-800">
      ...
      <div class="flex gap-1">
        <button :disabled="currentPage === 1" class="px-2 py-1 text-xs rounded bg-surface-800 text-surface-400 hover:text-surface-200 disabled:opacity-30" @click="currentPage--">Prev</button>
        <button v-for="p in visiblePages" :key="p" :class="p === currentPage ? 'bg-violet-600 text-white' : 'bg-surface-800 text-surface-400 hover:text-surface-200'" class="px-2.5 py-1 text-xs rounded" @click="currentPage = p">{{ p }}</button>
        <button :disabled="currentPage === totalPages" class="px-2 py-1 text-xs rounded bg-surface-800 text-surface-400 hover:text-surface-200 disabled:opacity-30" @click="currentPage++">Next</button>
      </div>
    </div>
  </div>
</template>
