  ...
  <div class="relative">
    <div v-for="(event, index) in events" :key="event.id || index" class="flex gap-4 pb-6 last:pb-0">
      <div class="flex flex-col items-center">
        <div :class="dotColor(event.to_status)" class="w-3 h-3 rounded-full ring-4 ring-surface-900 z-10 shrink-0" />
      ...
      <div class="flex-1 -mt-0.5 min-w-0">
        ...
        <p class="text-xs text-surface-500 mt-1">
          ...
          <span v-if="duration(index)" class="text-surface-600 ml-2">({{ duration(index) }})</span>
        </p>
        <p v-if="event.provider_ref" class="text-xs text-surface-600 mt-0.5 font-mono">ref: {{ event.provider_ref }}</p>
      </div>
    </div>
