...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'crypto_deposit_sessions_tenant_isolation') THEN
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'crypto_deposit_address_index_tenant_isolation') THEN
    CREATE POLICY crypto_deposit_address_index_tenant_isolation ON crypto_deposit_address_index
        FOR ALL
        TO settla_app
        USING (tenant_id = current_setting('app.current_tenant_id')::uuid)
        WITH CHECK (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'bank_deposit_sessions_tenant_isolation') THEN
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'bank_deposit_transactions_tenant_isolation') THEN
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'portal_users_tenant_isolation') THEN
...
GRANT SELECT, INSERT, UPDATE, DELETE ON crypto_deposit_address_index TO settla_app;
