  ...
  <div class="space-y-6">
    ...
    <template v-else-if="store.metrics">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <SummaryCard label="Transfers Today" :value="String(store.metrics.transfers_today)" />
        <SummaryCard label="Volume Today" :value="money.formatCompact(store.metrics.volume_today_usd, 'USD')" />
        <SummaryCard label="Completed" :value="String(store.metrics.completed_today)" variant="success" />
        <SummaryCard label="Failed" :value="String(store.metrics.failed_today)" :variant="store.metrics.failed_today > 0 ? 'danger' : 'default'" />
      </div>
      ...
      <div class="bg-surface-900 rounded-lg border border-surface-800 p-5">
        ...
        <LoadingSpinner v-if="store.chartLoading" size="sm" />
        <div v-else-if="store.chartBuckets.length" class="grid grid-cols-12 gap-px items-end h-32">
          ...
          >
            <div class="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-surface-800 text-surface-200 text-[10px] px-1.5 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none">
              {{ bucket.total }} txns
            </div>
          </div>
        </div>
        <EmptyState v-else title="No data" description="No transfer activity in this period" icon="&#x2015;" />
      </div>
    </template>

    <EmptyState v-else-if="store.error" :title="store.error" icon="!" />
  </div>
</template>
